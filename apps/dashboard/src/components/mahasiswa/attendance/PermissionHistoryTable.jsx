const PermissionHistoryTable = ({ records, filterDateStart, filterDateEnd, onFilterDateStartChange, onFilterDateEndChange, onResetFilter }) => {
    const filteredRecords = records.filter(r => {
        if (r.status !== 'Izin') return false;
    ...
    return (
        <section className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <h3 className="text-xl font-semibold text-[#1b0d0d]">Riwayat Izin</h3>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-50 border-b border-[#f3e7e7]">
                                ...
                                <th className="px-6 py-4 text-xs uppercase font-medium text-primary">Jenis Izin</th>
                                <th className="px-6 py-4 text-xs uppercase font-medium text-primary">Keterangan</th>
                                <th className="px-6 py-4 text-xs uppercase font-medium text-primary">Bukti</th>
                                <th className="px-6 py-4 text-xs uppercase font-medium text-primary">Status Validasi</th>
                            </tr>
                        </thead>
                            ...
                            {filteredRecords.length > 0 ? filteredRecords.map((record, index) => (
                                <tr key={record.id} className="hover:bg-gray-50/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${record.type === 'Sakit' ? 'bg-blue-50 text-blue-700 border-blue-100' : 'bg-blue-50 text-blue-700 border-blue-100'}`}>{record.type}</span>
                                    </td>
                                    <td className="px-6 py-4 text-sm text-gray-600 max-w-xs truncate" title={record.description}>{record.description}</td>
                                    <td className="px-6 py-4 text-sm">
                                        {record.proofLink ? (
                                            <a href={record.proofLink} target="_blank" rel="noopener noreferrer" className="text-primary hover:text-blue-700 hover:underline flex items-center gap-1 text-xs font-medium transition-colors">
                                                <span className="material-symbols-outlined notranslate text-[16px]">link</span>Lihat
                                            </a>
                                        ) : (
                                    ...
                                    <td className="px-6 py-4">
                                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-50 text-amber-700 border border-amber-200">Menunggu</span>
                                    </td>
                                </tr>
                            )) : (
                                <tr><td colSpan="6" className="px-6 py-8 text-center text-slate-500">Belum ada pengajuan izin.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
